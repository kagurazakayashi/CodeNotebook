...
book.pop(X) //删除指定位置的内容，x=0-x
  ...
  # 往列表里添加元素
  #
  #   append()              :     向数组里面添加新元素      例:array.append('霍元甲')                    array代表某个自定义的列表
  #
  #   extend()               :    向列表里面田间新元素       例:array.append(['霍元甲','李时珍'])     array代表某个自定义的列表
  #
  #   insert                    :    向列表里面田间新元素       例:array.insert(0,'霍元甲')　　　　　  array代表某个自定义的列表   0 代表插入的位置，列表的索引值
  #
  # 从列表里删除元素
  #
  #   remove()              :     从列表里删除元素              例:array.remove('霍元甲')　　　　　  array代表某个自定义的列表     括号里面填写的是元素名
  #
