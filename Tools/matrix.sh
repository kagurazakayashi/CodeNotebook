docker volume create matrix_v
docker volume create matrix_db
docker volume create matrix_redis
...
# redis 6379
docker run --name matrix_redis --net matrix --ip 172.20.0.4 -v matrix_redis:/data -d redis:latest

# 生成配置文件 到 /var/lib/docker/volumes/matrix_v/_data
docker run -it --rm -v matrix_v:/data -e SYNAPSE_SERVER_NAME=域名 -e SYNAPSE_REPORT_STATS=yes -e TZ=Asia/Shanghai matrixdotorg/synapse:latest generate

# 初始化PostgreSQL(最长密码99位)
docker run --name matrix_db -v matrix_db:/var/lib/postgresql/data --net matrix --ip 172.20.0.3 -e POSTGRES_PASSWORD=数据库密码 -d postgres:latest
# 进入docker db容器：
...
# 修改生成的 /var/lib/docker/volumes/matrix_v/_data/homeserver.yaml
# 一旦你有了一个有效的配置文件，你就可以按如下方式启动 synapse：

docker run -d --name matrix_synapse -v matrix_v:/data --net matrix --ip 172.20.0.2 -p 8008:8008 matrixdotorg/synapse:latest
