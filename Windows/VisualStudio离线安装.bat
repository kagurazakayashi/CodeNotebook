...
VisualStudioSetup.exe --layout D:\VisualStudioEnterprise2026Layout --lang zh-CN --all
@REM 包含其他语言
VisualStudioSetup.exe --layout D:\VisualStudioEnterprise2026Layout --lang zh-CN --lang zh-TW --lang en-US --lang ja-JP --all

@REM 创建后更新
@REM 出现较新版本时，可以通过互联网再次更新本地缓存，在原参数后面，添加“--useLatestInstaller”参数传递给引导程序，将其配置为最新的可用安装程序。
VisualStudioSetup.exe --layout D:\VisualStudioEnterprise2026Layout --lang zh-CN --lang zh-TW --lang en-US --lang ja-JP --useLatestInstaller --all

@REM 更新后删除旧版本
@REM 在“Archive”文件夹中，有一个或多个名为“GUID”的文件夹，其中每个都包含已过时的目录清单。
VisualStudioSetup.exe --layout D:\VisualStudioEnterprise2026Layout --clean D:\VisualStudioEnterprise2026Layout\Archive\1cd70189-fc55-4583-8ad8-a2711e928325\Catalog.json

@REM 修复文件
@REM 使用 --fix 执行与 --verify 相同的验证，并尝试修复标识的问题。
VisualStudioSetup.exe --layout D:\VisualStudioEnterprise2026Layout --fix
