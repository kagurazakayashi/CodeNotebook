# 自签泛域名证书
# 安装OpenSSL
sudo apt install openssl openssl-devel -y
...
# 制作CA公钥
# openssl req -sha256 -new -x509 -days 36500 -key CA.key -out CA.crt -config ./openssl.cnf
...
openssl req -sha256 -new -x509 -days 36500 -key CA.key -out CA.crt -config ./openssl.cnf -subj "/C=CN/ST=BJ/L=Beijing/O=Kagurazaka/OU=Yashi/CN=KagurazakaYashiSign CA/emailAddress=cxchope@163.com"

# 自签发泛域名证书
# 生成密钥
openssl genrsa -out uuu.moe.key 8192
# 生成证书签发请求
openssl req -new -sha256 -key uuu.moe.key -out uuu.moe.csr -config ./openssl.cnf -subj "/C=CN/ST=BJ/L=Beijing/O=Kagurazaka/OU=Yashi/CN=*.uuu.moe/emailAddress=cxchope@163.com"
# 查看签名请求文件信息
openssl req -in uuu.moe.csr -text
# 使用自签署的CA，签署crt
openssl ca -in uuu.moe.csr -md sha256 -days 36500 -out uuu.moe.crt -cert CA.crt -keyfile CA.key -config ./openssl.cnf
# 遇到报错: demoCA/index.txt: No such file or directory :
# 遇到报错: demoCA/serial: No such file or directory Unable to load number from ./demoCA/serial
...
# 检查部署是否正常
cat demoCA/index.txt && cat demoCA/serial
